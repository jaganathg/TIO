...
pub enum OHLCVError {
    ...
    InvalidOpen { open: Decimal, high: Decimal, low: Decimal },
    
    #[error("Invalid close price: Close ({close}) must be between High ({high}) and Low ({low})")]
    InvalidClose { close: Decimal, high: Decimal, low: Decimal },
...
impl OHLCV {
    ...
    pub fn from_array(
        ...
        let timestamp = DateTime::from_timestamp(data[0] as i64, 0)
            .ok_or_else(|| OHLCVError::MissingData { field: "timestamp".to_string() })?;
        
        let open = Decimal::from_f64_retain(data[1])
            .ok_or_else(|| OHLCVError::MissingData { field: "open".to_string() })?;
        let high = Decimal::from_f64_retain(data[2])
            .ok_or_else(|| OHLCVError::MissingData { field: "high".to_string() })?;
        let low = Decimal::from_f64_retain(data[3])
            .ok_or_else(|| OHLCVError::MissingData { field: "low".to_string() })?;
        let close = Decimal::from_f64_retain(data[4])
            .ok_or_else(|| OHLCVError::MissingData { field: "close".to_string() })?;
        let volume = Decimal::from_f64_retain(data[5])
            .ok_or_else(|| OHLCVError::MissingData { field: "volume".to_string() })?;
...
impl OHLCVBuilder {
    ...
    pub fn build(self) -> Result<OHLCV, OHLCVError> {
        let open = self.open.ok_or_else(|| OHLCVError::MissingData { field: "open".to_string() })?;
        let high = self.high.ok_or_else(|| OHLCVError::MissingData { field: "high".to_string() })?;
        let low = self.low.ok_or_else(|| OHLCVError::MissingData { field: "low".to_string() })?;
        let close = self.close.ok_or_else(|| OHLCVError::MissingData { field: "close".to_string() })?;
        let volume = self.volume.ok_or_else(|| OHLCVError::MissingData { field: "volume".to_string() })?;
        
        let mut ohlcv = OHLCV::new(self.symbol, self.timeframe, self.timestamp, open, high, low, close, volume)?;
...
mod tests {
    ...
    fn test_builder_pattern() {
        ...
        assert_eq!(ohlcv.open, Decimal::new(100,0));
        assert_eq!(ohlcv.get_metadata("source"), Some(&serde_json::Value::String("test".to_string())));
    }
    ...
    fn test_ordering_by_timestamp() {
        ...
        let ohlcv1 = OHLCV::new(symbol.clone(), TimeFrame::OneHour, timestamp1, 
                               Decimal::new(100,0), Decimal::new(105,0), Decimal::new(99,0), Decimal::new(103,0), Decimal::new(1000,0)).unwrap();
        let ohlcv2 = OHLCV::new(symbol, TimeFrame::OneHour, timestamp2,
                               Decimal::new(103,0), Decimal::new(108,0), Decimal::new(102,0), Decimal::new(106,0), Decimal::new(1200,0)).unwrap();
